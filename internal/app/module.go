...
type SettingGroup struct {
	...
	Order int
	// Icon 是该分组的图标名，取值来自 console/src/lib/icons.ts 的登记表。
	//
	// 分组自己声明图标，设置页的区块标题与命令面板入口因此都从分组列表推导——
	// 少一处需要手工同步的清单，也就不会再出现「后端有这一组、界面上没有入口」。
	Icon string
	// Toggle 是本组主开关的字段名，可留空。
	//
	// 设置页把所有分组铺在一页上，各组默认收起；声明了主开关的组，
	// 那个开关会提到区块标题栏上——「开放评论」「启用邮件发送」「开放注册」
	// 这类决定「这一整块要不要生效」的开关，是站长最常动的东西，
	// 让它需要先展开一层才够得着，就是把最常用的动作放到最深的地方。
	//
