...
type SettingGroup struct {
	...
	// Hidden 为真时这一组不进侧边栏，表单由别的页面承载（见 internal/account 与用户页）。
	//
	// 设置内容本身一切照旧：存储、接口、公开字段白名单都不受影响，
	// 只是不再单独占一个菜单项。用处是**避免同一件事在侧边栏里出现两次**——
	// 「开放注册」管的是谁能成为用户，把它和「用户」分成两个入口，
	// 站长看到的就是两个都像、又都不是的东西。
	//
	// 隐藏的组仍然能通过 /settings/<名字> 直接打开：地址是接口的一部分，
	// 不是菜单的附属品，而一个藏起来的分组不该同时也变成一个打不开的页面。
	Hidden bool
}
