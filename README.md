...
1. 下载当前平台的发布包，**按 GitHub 接口给出的 SHA-256 核对**（老版本没有这个字段时按 `checksums.txt`），对不上直接中止
...
检查新版本要连 `api.github.com`。下载时先测速：几个加速地址和 GitHub 直连各下开头 256 KB，
从最快的那个分 4 条连接下载，断了从断点接着下；这个来源失败或内容和摘要对不上，就换下一个。
内置的加速地址是几个公共的 GitHub 下载加速服务，后台「关于」页可以改成自己的，或者全删掉只直连。
这些地址改不了安装包：摘要取自 GitHub 接口，下完对不上就不装。访问 GitHub 要走代理时，
给进程设上 `HTTPS_PROXY`，检查与下载都会走它。
...
└── cover.webp          # 可选，后台主题页的特色图（也可以是 png、jpg；4:3，建议 1200×900）
```
