...
cover.webp          可选  后台主题页卡片上的特色图，也可以是 cover.png 或 cover.jpg；4:3，建议 1200×900
templates/          必需  模板目录
  layouts/          可选  外层骨架
  partials/         可选  可复用片段
...
代价是模板语法不如 Jinja 顺手，Lumo 用 layout/partial 约定和一套函数库来补。
...
单次调用最多返回 **100 条**，超过的部分截断。结果有缓存，页眉页脚取同一个菜单不会查两次库，
下一个访客也不必再查；站长改了文章、菜单或设置，缓存立刻作废，刷新就能看到。
多台实例共用一个数据库时，别的实例上的改动最迟 30 秒后可见。
...
| `.Find.Posts.List n` | 前 n 篇，置顶在前，与 `/posts`、分类页同序；首页主列表用它 |
...
插件往前台放东西（样式、脚本、统计代码、正文后的推荐、评论区下的订阅框、侧栏小组件），
全靠主题在页面里留好的口子。**主题不调这些，插件就进不了前台**。
...
| 调用 | 放在哪 | 插件会放什么 |
|---|---|---|
| `{{ .Slot "head" }}` | `</head>` 之前，主题样式表之后 | 插件的样式表、meta 标签 |
| `{{ .Slot "content.before" }}` | 文章与页面的正文之前 | 如阅读提示 |
| `{{ .Slot "content.after" }}` | 正文之后 | 如打赏、推荐阅读 |
| `{{ .Slot "comments.after" }}` | 评论区之后 | 如订阅框 |
| `{{ .Slot "footer" }}` | `</body>` 之前，主题脚本之后 | 页尾内容与插件的脚本 |

`head` 与 `footer` 每个页面都该有，写在 layout 里；另外三个只在文章与页面上有意义。
内容已由插件模块净化过（脚本只会是插件包里的文件，事件属性与内联脚本一律进不来），原样输出，
不必也不要再套 `safeHTML`。没有插件往里放东西时输出空串。

除 `head` 外，每个插件的内容都包在 `<div class="plugin-slot" data-plugin="插件名" data-slot="插槽名">` 里，
需要时可以按它给间距。
...
插件可以提供侧栏小组件，id 形如 `插件名/小组件名`。`.Widget` 渲染一个，插件没启用或这次没内容时返回 nil：

```html
{{ with .Widget "visit-stats/counter" }}
...
让站长自己挑：在侧栏设置的条目里加一个字段，写 `x-widget: plugin-widget`，后台会渲染成
「按插件分组的下拉」，存下来的就是那个 id。内置主题「墨」的侧边栏就是这么做的（小组件类型「插件小组件」）：
...
widget:
  ...
  title: 哪个插件小组件
  maxLength: 130
  x-widget: plugin-widget
  x-show-if: { field: type, op: eq, value: plugin }
```

插件停用后，已存的 id 保留，前台那一条暂不显示；重新启用就回来了。

### 短代码

作者在正文里写 `[名字 参数="值"]`，由提供它的插件展开。主题什么都不用做——正文 `.Post.Content`
交到模板时已经展开好了。只在文章与页面的详情页展开，列表、摘要与订阅源里保持原样。
...
升级程序时，没改过的副本会自动换成新版本；改过的原样保留，要新版本得先备份改动再恢复出厂。
判断依据是解压时写下的 `.factory.json`（每个文件的 SHA-256），别删它。
